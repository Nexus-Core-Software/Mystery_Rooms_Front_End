...
function PuzzleRoom3D({ roomData, onExit, forcePlayerView }) {
  ...
  const [completedPuzzles, setCompletedPuzzles] = useState([])  // lista de puzzles completados
   ...
   */
  const updateRoomInStorage = (delta) => {
    try {
      const rooms = JSON.parse(localStorage.getItem('createdRooms') || '[]')
      const idx = rooms.findIndex(r => r.roomName === effectiveRoomData.roomName)
      if (idx !== -1) {
        rooms[idx].playerCount = Math.max(0, (rooms[idx].playerCount || 1) + delta)
        localStorage.setItem('createdRooms', JSON.stringify(rooms))
        setRoomPlayerCount(rooms[idx].playerCount)
      }
    } catch (error) {
  ...
  /**
   * Elimina la sala de localStorage y sincronización
   * SOLO para el creador al cerrar sala
   */
  const removeRoomFromStorage = () => {
    try {
      const rooms = JSON.parse(localStorage.getItem('createdRooms') || '[]')
      const filtered = rooms.filter(r => r.roomName !== effectiveRoomData.roomName)
      localStorage.setItem('createdRooms', JSON.stringify(filtered))
      localStorage.removeItem(`PLAYERS_SYNC:${effectiveRoomData.roomName}`)
    } catch (error) {
  ...
  const closeRoom = () => {
    ...
    try {
      // Remover la sala de createdRooms
  ...
  useEffect(() => {
    ...
    const actualPlayerCount = playerProgress.length || 1
    const playerNames = playerProgress.map(player => player.name)
  ...
  }, [onExit, puzzleStations, effectiveRoomData.roomName, myPlayerId, safePlayerCount]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const updateActivity = () => {
      try {
        ...
        if (savedRooms && effectiveRoomData.roomName !== 'Sala 3D') {
          const rooms = JSON.parse(savedRooms)
          const roomIndex = rooms.findIndex(room => room.roomName === effectiveRoomData.roomName)
          if (roomIndex !== -1) {
  ...
  useEffect(() => {
    const checkPlayerStatus = () => {
      try {
        ...
        const rooms = JSON.parse(localStorage.getItem('createdRooms') || '[]')
        const roomExists = rooms.some(r => r.roomName === effectiveRoomData.roomName)

        if (!roomExists) {
          // La sala fue cerrada - desconectar inmediatamente
    ...
    const interval = setInterval(checkPlayerStatus, 200)
    return () => clearInterval(interval)
  }, [effectiveRoomData.roomName, myPlayerId, onExit])
